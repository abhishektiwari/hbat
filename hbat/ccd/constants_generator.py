...
class CCDConstantsGenerator:
    ...
    def write_residue_bonds_constants(
        ...
        try:
            ...
            print(f"Successfully wrote bond data for {len(residue_bonds)} residues to {output_path}")
            return True
    ...
    def _write_file_header(self, f):
        ...
        f.write('This module contains bond connectivity information for standard residues\n')
        f.write('extracted from the Chemical Component Dictionary (CCD). This data is used\n')
    ...
    def _write_helper_functions(self, f):
        ...
        f.write('def get_residue_bonds(residue: str) -> List[Dict[str, Union[str, bool]]]:\n')
        f.write('    """\n')
        ...
        f.write('        \n')
        f.write('    Returns:\n')
        f.write('        List of bond dictionaries with atom1, atom2, order, and aromatic info\n')
        f.write('    """\n')
        ...
        f.write('    """\n')
        f.write('    aromatic_count = RESIDUE_BONDS.get(residue, {}).get("aromatic_bonds", 0)\n')
    ...
    def _write_summary_constants(self, f, residue_bonds: Dict[str, Dict]):
        ...
        aromatic_residues = len([r for r in residue_bonds.values() if r["aromatic_bonds"] > 0])
    ...
    def print_analysis_report(self, report: Dict):
        ...
        for i, (residue, summary) in enumerate(report['residue_summaries'].items()):
            if i >= 3:  # Show only first 3 as examples
                break
            print(f"  {residue}: {summary['atom_count']} atoms, {summary['bond_count']} bonds")