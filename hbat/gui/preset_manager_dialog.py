...
class PresetManagerDialog:
    ...
    def _create_load_tab(self, notebook):
        ...
        self.preset_tree = ttk.Treeview(list_frame, columns=columns, show="tree headings", height=12)
        ...
        v_scrollbar = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.preset_tree.yview)
        h_scrollbar = ttk.Scrollbar(list_frame, orient=tk.HORIZONTAL, command=self.preset_tree.xview)
        self.preset_tree.configure(yscrollcommand=v_scrollbar.set, xscrollcommand=h_scrollbar.set)
        ...
        ttk.Button(
            action_frame, 
            text="Browse...", 
            command=self._browse_for_preset
        ).pack(side=tk.LEFT, padx=(0, 5))
        
        ttk.Button(
            action_frame, 
            text="Refresh", 
            command=self._refresh_preset_list
    ...
    def _create_save_tab(self, notebook):
        ...
        self.name_var = tk.StringVar(value=f"Custom Preset {datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        preview_frame = ttk.LabelFrame(save_frame, text="Current Parameters", padding="10")
        preview_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 20))
        
        # Create text widget for parameters preview
        self.preview_text = tk.Text(preview_frame, height=10, width=60, font=("Courier", 9))
    ...
    def _create_manage_tab(self, notebook):
        ...
        instructions = ttk.Label(
            manage_frame,
            text="Manage your preset files:",
            font=("TkDefaultFont", 10)
        )
        instructions.pack(anchor=tk.W, pady=(0, 20))
        
        # Directory info
        dir_frame = ttk.LabelFrame(manage_frame, text="Preset Directories", padding="10")
        ...
        ttk.Label(user_dir_frame, text=user_dir, font=("Courier", 8)).pack(side=tk.LEFT, fill=tk.X, expand=True)
        ttk.Button(
            user_dir_frame, 
            text="Open", 
        ...
        ttk.Label(example_dir_frame, text=example_dir, font=("Courier", 8)).pack(side=tk.LEFT, fill=tk.X, expand=True)
        ttk.Button(
            example_dir_frame, 
            text="Open", 
    ...
    def _refresh_preset_list(self):
        ...
        self._load_presets_from_directory("Examples", self._get_example_presets_directory())
    ...
    def _load_presets_from_directory(self, category: str, directory: str):
        ...
        category_node = self.preset_tree.insert("", "end", text=category, values=("", "", "", ""))
                ...
                if filename.endswith(('.hbat', '.json')):
                    ...
                    try:
                        ...
                        if created:
                            ...
                            formatted_date = datetime.fromtimestamp(mtime).strftime("%Y-%m-%d %H:%M")
    ...
    def _browse_for_preset(self):
        ...
        filename = filedialog.askopenfilename(
            ...
            filetypes=[("HBAT Presets", "*.hbat"), ("JSON files", "*.json"), ("All files", "*.*")]
        )
    ...
    def _save_current_preset(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"Preset saved successfully to:\n{os.path.basename(filename)}")
                self._refresh_preset_list()
    ...
    def _import_preset(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"Preset imported successfully to:\n{dest_filename}")
                self._refresh_preset_list()
