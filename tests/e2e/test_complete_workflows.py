...
class TestCompleteAnalysisWorkflows:
    ...
    def test_pdb_fixing_workflow(self, sample_pdb_file, expected_results):
        ...
        assert hydrogen_count >= 700, (
            f"Expected >= 700 hydrogens after fixing, got {hydrogen_count}"
        )
    ...
    def test_parameter_customization_workflow(self, sample_pdb_file):
        ...
        strict_params = AnalysisParameters(
            hb_distance_cutoff=3.0, hb_angle_cutoff=140.0, analysis_mode="local"
        )

        # Create permissive parameters
        permissive_params = AnalysisParameters(
            hb_distance_cutoff=4.0, hb_angle_cutoff=110.0, analysis_mode="complete"
        )
        ...
        strict_stats = {
            ...
            "total_interactions": len(strict_analyzer.hydrogen_bonds)
            + len(strict_analyzer.halogen_bonds)
            + len(strict_analyzer.pi_interactions),
        }
        permissive_stats = {
            ...
            "total_interactions": len(permissive_analyzer.hydrogen_bonds)
            + len(permissive_analyzer.halogen_bonds)
            + len(permissive_analyzer.pi_interactions),
        }
        ...
        assert (
            permissive_stats["total_interactions"] >= strict_stats["total_interactions"]
        )
...
class TestResultsExportWorkflows:
    ...
    def test_summary_statistics_workflow(self, sample_pdb_file):
        ...
        assert (
            summary["carbonyl_interactions"]["count"] == stats["carbonyl_interactions"]
        )
...
class TestLargeFileWorkflows:
    ...
    def test_large_structure_analysis_workflow(self, sample_pdb_file, expected_results):
        ...
        assert memory_usage < 100 * 1024 * 1024, (
            f"Memory usage too high: {memory_usage} bytes"
        )
...
class TestCooperativityWorkflows:
    ...
    def test_cooperativity_analysis_workflow(self, sample_pdb_file):
        ...
        if len(chains) > 0:
            assert summary.get("cooperativity_chains", {}).get("count", 0) == len(
                chains
            )
            ...
            for chain in chains[:3]:  # Test first 3 chains
                ...
                for interaction in chain.interactions:
                    ...
                    assert interaction.interaction_type in ["H-Bond", "X-Bond", "π-Int"]
                    ...
                    "interactions": [
                        {
                            ...
                            "donor_residue": getattr(
                                interaction, "donor_residue", "N/A"
                            ),
                            "acceptor_residue": getattr(
                                interaction, "acceptor_residue", "N/A"
                            ),
