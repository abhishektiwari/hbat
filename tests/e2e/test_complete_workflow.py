...
class TestCompleteWorkflow:
    ...
    def test_full_analysis_pipeline_7nwd(self):
        ...
        assert pi_pi > 0, (
            "No π-π interactions detected in 7nwd (expected for nucleic acid structure)"
        )

        total_interactions = h_bonds + x_bonds + pi_pi + carbonyl + n_pi
        assert total_interactions > 10, (
            f"Too few total interactions: {total_interactions}"
        )
        ...
        print("✓ 7nwd.pdb analysis complete:")
        print(
            f"  H-bonds: {h_bonds}, X-bonds: {x_bonds}, π-π: {pi_pi}, Carbonyl: {carbonyl}, n→π*: {n_pi}"
        )
    ...
    def test_full_analysis_pipeline_6rsa(self):
        ...
        assert carbonyl > 0, (
            "No carbonyl interactions detected in 6rsa (expected for protein)"
        )
        ...
        if carbonyl > 0:
            for interaction in analyzer.carbonyl_interactions[:5]:  # Check first 5
                assert 2.5 <= interaction.distance <= 4.0, (
                    f"Invalid carbonyl distance: {interaction.distance}"
                )
                assert 90.0 <= interaction.burgi_dunitz_angle <= 130.0, (
                    f"Invalid Bürgi-Dunitz angle: {interaction.burgi_dunitz_angle}"
                )
                assert hasattr(interaction, "interaction_classification")

        print("✓ 6rsa.pdb analysis complete:")
        print(
            f"  H-bonds: {h_bonds}, X-bonds: {x_bonds}, π-π: {pi_pi}, Carbonyl: {carbonyl}, n→π*: {n_pi}"
        )
    ...
    def test_full_analysis_pipeline_n_pi_structures(self):
        ...
        for pdb_file in ["example_pdb_files/1luc.pdb", "example_pdb_files/2cdg.pdb"]:
            ...
            n_pi = len(analyzer.n_pi_interactions) if analyzer.n_pi_interactions else 0
            assert n_pi > 0, (
                f"No n→π* interactions detected in {pdb_file} (expected after fix)"
            )

            # Step 3: Verify n→π* interaction properties
            for interaction in analyzer.n_pi_interactions[:3]:  # Check first 3
                assert 2.5 <= interaction.distance <= 3.6, (
                    f"Invalid n→π* distance: {interaction.distance}"
                )
                assert 0.0 <= interaction.angle_to_plane <= 45.0, (
                    f"Invalid n→π* angle: {interaction.angle_to_plane}"
                )
                assert interaction.donor_element in ["O", "N", "S"], (
                    f"Invalid donor element: {interaction.donor_element}"
                )
...
class TestWorkflowRobustness:
    ...
    def test_analysis_with_all_interaction_types_enabled(self):
        ...
        assert abs(total_from_summary - manual_total) <= 5, (
            f"Summary total ({total_from_summary}) differs significantly from manual count ({manual_total})"
        )
...
class TestPerformanceAndScaling:
    ...
    def test_large_structure_analysis(self):
        ...
        # Increased to 150s to accommodate GitHub Actions and varying system loads
        assert analysis_time < 150.0, f"Analysis too slow: {analysis_time:.2f}s"
        ...
        assert total_interactions > 20, (
            f"Too few interactions for large structure: {total_interactions}"
        )

        print(
            f"✓ Large structure analysis: {analysis_time:.2f}s, {total_interactions} interactions"
        )
    ...
    def test_multiple_structure_analysis(self):
        ...
        assert total_interactions > 50, (
            f"Expected more total interactions: {total_interactions}"
        )
...
class TestBackwardCompatibility:
    ...
    def test_existing_functionality_unchanged(self):
        ...
        print(
            f"✓ Backward compatibility: {summary['hydrogen_bonds']['count']} H-bonds detected"
        )
