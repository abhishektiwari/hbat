...
class TestCLICIFWorkflows:
    ...
    def test_cli_cif_analysis_without_fixing(self):
        ...
        assert fixed_file is None, (
            "No fixed file should be created when fixing is disabled"
        )
    ...
    def test_cli_cif_export_json(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            parser = create_parser()
            args = parser.parse_args(["example_pdb_files/6RSA.cif", "-o", output_file])
    ...
    def test_cli_cif_export_csv(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            parser = create_parser()
            args = parser.parse_args(
                ["example_pdb_files/6RSA.cif", "--csv", base_filename]
            )
    ...
    def test_cli_cif_vs_pdb_equivalence(self):
        ...
        # Analyze CIF
        parser = create_parser()
        args_cif = parser.parse_args(["example_pdb_files/6RSA.cif"])
        ...
        # Analyze PDB
        args_pdb = parser.parse_args(["example_pdb_files/6rsa.pdb"])
    ...
    def test_cli_cif_complete_workflow_with_fixing(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "hydrogen_bonds" in data
            assert len(data["hydrogen_bonds"]) > 0, (
                "No H-bonds detected in complete workflow"
            )
    ...
    def test_cli_cif_multi_file_comparison_with_pdbfixer(self):
        ...
        for file_id, cif_file, pdb_file in test_files:
            ...
            args_cif = parser.parse_args(
                [cif_file, "--fix-pdb", "--fix-method", "pdbfixer"]
            )
            params_cif = load_parameters_from_args(args_cif)
            analyzer_cif = NPMolecularInteractionAnalyzer(params_cif)
            ...
            args_pdb = parser.parse_args(
                [pdb_file, "--fix-pdb", "--fix-method", "pdbfixer"]
            )
            params_pdb = load_parameters_from_args(args_pdb)
            analyzer_pdb = NPMolecularInteractionAnalyzer(params_pdb)
            ...
            tolerance = (
                max(cif_count, pdb_count) * 0.35
            )  # 35% tolerance for stochastic hydrogen placement variation
        ...
        print("\n" + "=" * 100, file=sys.stderr)
        print(
            "Multi-file PDBFixer Comparison Results (6rsa, 1crn, 4x21)", file=sys.stderr
        )
        print("=" * 100, file=sys.stderr)
        print(
            f"{'File':<10} {'Metric':<15} {'CIF':<12} {'PDB':<12} {'Diff':<12} {'Status':<10}",
            file=sys.stderr,
        )
        print("-" * 100, file=sys.stderr)
        ...
        for file_id, data in results.items():
            ...
            print(
                f"{'':10} {'X-bonds':<15} {cif_xb:<12} {pdb_xb:<12} {diff_xb:<12}",
                file=sys.stderr,
            )
            print(
                f"{'':10} {'π-interact':<15} {cif_pi:<12} {pdb_pi:<12} {diff_pi:<12}",
                file=sys.stderr,
            )
