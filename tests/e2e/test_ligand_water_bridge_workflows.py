...
class TestLigandInteractionsWorkflows:
    ...
    @pytest.mark.parametrize(
        "test_data", LIGAND_INTERACTION_TEST_DATA, ids=lambda x: x["pdb_name"]
    )
    def test_ligand_interactions_detection(self, test_data, expected_results):
        ...
        assert hasattr(analyzer, "ligand_interactions"), (
            f"{pdb_name}: Should have ligand_interactions attribute"
        )
        assert analyzer.ligand_interactions is not None, (
            f"{pdb_name}: ligand_interactions should not be None"
        )
        assert analyzer.ligand_interactions.interactions, (
            f"{pdb_name}: Should have ligand interactions"
        )
        assert (
            len(analyzer.ligand_interactions.interactions)
            >= test_data["expected_min_interactions"]
        ), (
        ...
        assert "ligand_interactions" in summary, (
            f"{pdb_name}: Summary should include ligand_interactions"
        )
        lig_stats = summary["ligand_interactions"]
        assert lig_stats["count"] > 0, (
            f"{pdb_name}: Should have ligand interaction count > 0"
        )
        ...
        if test_data["requires_water_bridge"]:
            assert len(analyzer.water_bridges) > 0, (
                f"{pdb_name}: Should have water bridges"
            )
            ...
            if unique_ligands == 1:
                ...
                for wb in analyzer.water_bridges:
                    ...
                    if ligand_res in donor_res or ligand_res in acceptor_res:
                        ...
                        break
                assert ligand_involved_in_wb, (
                    f"{pdb_name}: At least one water bridge should involve the ligand"
                )
...
class TestWaterBridgeWorkflows:
    ...
    @pytest.mark.parametrize(
        "test_data", WATER_BRIDGE_TEST_DATA, ids=lambda x: x["pdb_name"]
    )
    def test_water_bridge_detection(self, test_data, expected_results):
        ...
        assert hasattr(analyzer, "water_bridges"), (
            f"{pdb_name}: Should have water_bridges attribute"
        )
        assert len(analyzer.water_bridges) > 0, (
            f"{pdb_name}: Should detect water bridges"
        )

        # Verify bridge count is within expected range
        wb_count = len(analyzer.water_bridges)
        assert (
            test_data["expected_min_bridges"]
            <= wb_count
            <= test_data["expected_max_bridges"]
        ), (
        ...
        assert "water_bridges" in summary, (
            f"{pdb_name}: Summary should include water_bridges"
        )
        ...
        for wb in analyzer.water_bridges:
            ...
            assert hasattr(wb, "water_residues"), (
                f"{pdb_name}: Bridge should have water_residues"
            )
            assert hasattr(wb, "bridge_length"), (
                f"{pdb_name}: Bridge should have bridge_length"
            )
            assert hasattr(wb, "get_donor_residue"), (
                f"{pdb_name}: Bridge should have get_donor_residue method"
            )
            assert hasattr(wb, "get_acceptor_residue"), (
                f"{pdb_name}: Bridge should have get_acceptor_residue method"
            )
            assert hasattr(wb, "get_donor_acceptor_distance"), (
                f"{pdb_name}: Bridge should have get_donor_acceptor_distance method"
            )

            # Verify values are reasonable
            assert len(wb.water_residues) > 0, (
                f"{pdb_name}: Bridge should have water residues"
            )
            assert wb.bridge_length > 0, f"{pdb_name}: Bridge length should be positive"
            assert wb.get_donor_acceptor_distance() > 0, (
                f"{pdb_name}: Donor-acceptor distance should be positive"
            )
...
class TestLigandWaterBridgeIntegration:
    ...
    def test_ligand_interaction_and_water_bridge_separation(
        self, pdb_name, expected_results
    ):
    ...
    def test_openbabel_analysis_consistency(self, pdb_name, expected_results):
        ...
        assert "hydrogen_bonds" in summary, (
            f"{pdb_name}: Summary should have hydrogen_bonds"
        )
        assert "water_bridges" in summary, (
            f"{pdb_name}: Summary should have water_bridges"
        )
        assert "ligand_interactions" in summary, (
            f"{pdb_name}: Summary should have ligand_interactions"
        )
        ...
        for inter_type in interaction_types:
            assert inter_type in summary, (
                f"{pdb_name}: Summary should have {inter_type}"
            )
            assert summary[inter_type]["count"] >= 0, (
                f"{pdb_name}: {inter_type} count should be non-negative"
            )
