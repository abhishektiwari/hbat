...
class TestGeometryCutoffsDialogLogic:
    ...
    def test_parameter_storage_initialization(self):
        ...
        try:
            from hbat.gui.geometry_cutoffs_dialog import GeometryCutoffsDialog  # noqa: F401
            from hbat.constants.parameters import AnalysisParameters, ParametersDefault  # noqa: F401
    ...
    def test_parameter_get_value_logic(self):
        ...
        try:
            ...
            param_values = {"hb_distance": 3.2, "hb_angle": 125.0}

            # Test stored values are returned
            assert (
                get_value(
                    param_values, "hb_distance", ParametersDefault.HB_DISTANCE_CUTOFF
                )
                == 3.2
            )
            assert (
                get_value(param_values, "hb_angle", ParametersDefault.HB_ANGLE_CUTOFF)
                == 125.0
            )
            ...
            assert (
                get_value(
                    param_values, "xb_distance", ParametersDefault.XB_DISTANCE_CUTOFF
                )
                == ParametersDefault.XB_DISTANCE_CUTOFF
            )
