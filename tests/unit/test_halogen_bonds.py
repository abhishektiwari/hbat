...
class TestHalogenBondVdWDistanceCriteria:
    ...
    def test_vdw_sum_all_halogen_acceptor_pairs(self, analyzer):
        ...
        halogens = [("F", "FLU"), ("CL", "CLU"), ("BR", "BRU"), ("I", "IOU")]
        acceptors = [
            ("N", "ASN"),
            ("O", "GLY"),
            ("S", "CYS"),
        ...
        for (hal_elem, hal_res), (acc_elem, acc_res) in [
            (h, a) for h in halogens for a in acceptors
        ]:
            # Create atoms
            hal_atom = Atom(
                serial=1,
...
class TestHalogenBondDetectionLogic:
    ...
    def mock_analyzer(self):
        ...
        analyzer = NPMolecularInteractionAnalyzer(
            AnalysisParameters(xb_distance_cutoff=3.5)
        )
    ...
    def test_distance_validation_in_loop(self, mock_analyzer):
        ...
        meets_criteria_fail = (distance_too_large <= vdw_sum) or (
            distance_too_large <= fixed_cutoff
        )
