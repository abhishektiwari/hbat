...
class TestParsingEquivalence:
    ...
    def test_cif_vs_pdb_residue_equivalence(self):
        ...
        for chain in common_chains:
            ...
            protein_res_names = {'ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLU', 'GLN',
                                'GLY', 'HIS', 'ILE', 'LEU', 'LYS', 'MET', 'PHE',
                                'PRO', 'SER', 'THR', 'TRP', 'TYR', 'VAL'}

            pdb_protein_res = {k: v for k, v in parser_pdb.residues.items()
                              if v.chain_id == chain and v.name in protein_res_names}
            cif_protein_res = {k: v for k, v in parser_cif.residues.items()
                              if v.chain_id == chain and v.name in protein_res_names}
    ...
    def test_cif_vs_pdb_chain_equivalence(self):
        ...
        assert len(chains_pdb & chains_cif) > 0, \
            f"No common chains between PDB and CIF"
...
class TestAnalysisEquivalence:
    ...
    def test_cif_vs_pdb_all_interactions_equivalence(self):
        ...
        variance = 0.15  # Allow ±15% variance for files with different chain organization
    ...
    def test_cif_vs_pdb_interaction_details_equivalence(self):
        ...
        assert len(analyzer_pdb.hydrogen_bonds) > 0, \
            "No hydrogen bonds detected in PDB"
        assert len(analyzer_cif.hydrogen_bonds) > 0, \
            "No hydrogen bonds detected in CIF"
...
class TestParsingDetails:
    ...
    def test_water_molecule_parsing_equivalence(self):
        ...
        water_pdb = [res for key, res in parser_pdb.residues.items()
                     if res.name == 'HOH' and res.chain_id in common_chains]
        water_cif = [res for key, res in parser_cif.residues.items()
                     if res.name == 'HOH' and res.chain_id in common_chains]
