...
class TestCIFPDBFixing:
    ...
    def test_cif_and_pdb_fixing_similar_results(self):
        ...
        assert abs(cif_hb - pdb_hb) <= 5, f"H-bonds differ more than expected: CIF={cif_hb}, PDB={pdb_hb}"
        ...
        assert abs(cif_pi - pdb_pi) <= 1, f"π interactions differ significantly: CIF={cif_pi}, PDB={pdb_pi}"

        # Carbonyl interactions should match exactly (not affected by hydrogen placement)
        assert len(analyzer_cif.carbonyl_interactions) == len(analyzer_pdb.carbonyl_interactions)
