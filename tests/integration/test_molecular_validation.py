...
class TestMolecularValidation:
    ...
    def test_specific_hydrogen_bond_measurements(self, sample_pdb_file):
        ...
        print("=" * 100)
        print(
            f"{'Donor':<10} {'D-Elem':<8} {'Acceptor':<10} {'A-Elem':<8} {'D-A (Å)':<10} {'H-A (Å)':<10} {'D-H-A (°)':<10} {'H-Bond':<10}"
        )
        print("-" * 100)
        ...
        for donor, acceptor in test_pairs:
            if (donor, acceptor) in found_pairs:
                ...
                print(
                    f"{donor:<10} {donor_elem:<8} {acceptor:<10} {acceptor_elem:<8} {da_distance:<10.3f} {ha_distance:<10.3f} {dha_angle:<10.1f} {'Yes':<10}"
                )

                # Validate measurements
                assert da_distance > 0, (
                    f"D-A distance should be positive for {donor}/{acceptor}"
                )
                assert ha_distance > 0, (
                    f"H-A distance should be positive for {donor}/{acceptor}"
                )
                assert 0 <= dha_angle <= 180, (
                    f"D-H-A angle should be between 0-180° for {donor}/{acceptor}"
                )

                # Check that distances are within expected ranges
                assert 2.0 <= da_distance <= 4.0, (
                    f"D-A distance {da_distance:.3f} outside expected range for {donor}/{acceptor}"
                )
                assert 1.0 <= ha_distance <= 3.5, (
                    f"H-A distance {ha_distance:.3f} outside expected range for {donor}/{acceptor}"
                )
                assert dha_angle >= 90, (
                    f"D-H-A angle {dha_angle:.1f} too acute for {donor}/{acceptor}"
                )
            else:
                ...
                if donor_atom and acceptor_atom:
                    ...
                    print(
                        f"{donor:<10} {donor_elem:<8} {acceptor:<10} {acceptor_elem:<8} {da_distance:<10.3f} {'N/A':<10} {'N/A':<10} {'No':<10}"
                    )
                else:
                    print(
                        f"{donor:<10} {'N/A':<8} {acceptor:<10} {'N/A':<8} {'N/A':<10} {'N/A':<10} {'N/A':<10} {'No':<10}"
                    )
        ...
        if reversed_pairs:
            ...
            print("=" * 100)
            print(
                f"{'Expected D':<12} {'Expected A':<12} {'Actual D':<10} {'Actual A':<10} {'D-Elem':<8} {'A-Elem':<8}"
            )
            print("-" * 100)
            for (expected_d, expected_a), hb in reversed_pairs.items():
                print(
                    f"{expected_d:<12} {expected_a:<12} {hb.donor.serial:<10} {hb.acceptor.serial:<10} {hb.donor.element:<8} {hb.acceptor.element:<8}"
                )
            print("=" * 100)
    ...
    def test_molecular_geometry_validation(self, sample_pdb_file):
        ...
        for hb in hbonds:
            ...
            assert hb.donor_acceptor_distance > hb.distance, (
                "D-A should be > H-A distance"
            )

            # Angle should be in radians and reasonable
            angle_deg = math.degrees(hb.angle)
            assert 90 <= angle_deg <= 180, (
                f"D-H-A angle {angle_deg:.1f}° outside reasonable range"
            )

            # Distance constraints
            assert 1.0 <= hb.distance <= 3.5, (
                f"H-A distance {hb.distance:.3f}Å outside typical range"
            )
            assert 2.0 <= hb.donor_acceptor_distance <= 4.0, (
                f"D-A distance {hb.donor_acceptor_distance:.3f}Å outside typical range"
            )
    ...
    def test_interaction_distribution_validation(self, sample_pdb_file):
        ...
        if total_interactions > 0:
            ...
            assert hb_ratio >= 0.3, (
                f"H-bond ratio {hb_ratio:.2f} unusually low for protein structure"
            )
    ...
    def test_reference_structure_consistency(self, sample_pdb_file):
        ...
        assert stats1["hydrogen_bonds"]["count"] == stats2["hydrogen_bonds"]["count"], (
            "H-bond counts should be consistent"
        )
        assert (
            stats1["pi_interactions"]["count"] == stats2["pi_interactions"]["count"]
        ), "π interaction counts should be consistent"
        assert stats1["total_interactions"] == stats2["total_interactions"], (
            "Total interaction counts should be consistent"
        )
        ...
        for i in range(min(5, len(hbonds1))):
            ...
            assert abs(hb1.distance - hb2.distance) < 1e-6, (
                f"H-bond {i} distance should be identical"
            )
            assert abs(hb1.angle - hb2.angle) < 1e-6, (
                f"H-bond {i} angle should be identical"
            )
            assert hb1.donor.serial == hb2.donor.serial, (
                f"H-bond {i} donor should be identical"
            )
            assert hb1.acceptor.serial == hb2.acceptor.serial, (
                f"H-bond {i} acceptor should be identical"
            )
