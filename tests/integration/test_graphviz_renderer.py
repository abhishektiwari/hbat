...
class TestGraphVizRendererIntegration(unittest.TestCase):
    ...
    def test_edge_styling(self, mock_available):
        ...
        G.add_edge(
            ...
            "B",
            interaction=Mock(
                interaction_type="Hydrogen Bond", distance=2.8, angle=2.96
            ),
        )
        ...
        G.add_edge(
            "B",
            "C",
            interaction=Mock(interaction_type="Halogen Bond", distance=3.2, angle=2.79),
        )
    ...
    def test_configuration_integration(self, mock_available):
        ...
        self.config.set_graphviz_engine(
            "neato"
        )  # Note: engine is now always "dot" in renderer
        ...
        self.assertIn(
            'shape="box"', dot_string
        )  # Just check for shape="box", not the full node line
    ...
    def test_mouse_scroll_bindings(self, mock_available):
        ...
        for binding in expected_bindings:
            ...
            self.assertTrue(
                any(binding in str(b) for b in bindings) or binding in str(bindings)
            )
