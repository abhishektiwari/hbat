...
class MockInteraction:
    ...
    def __init__(
        self, donor, acceptor, interaction_type="H-Bond", distance=2.8, angle=2.96
    ):
...
class TestGraphVizWorkflows(unittest.TestCase):
    """Integration tests for GraphViz GUI component coordination."""
    ...
    def _create_sample_chain(self):
        ...
        interactions = [MockInteraction(ser_og, thr_n)]
    ...
    def test_complete_workflow_with_graphviz(self, mock_available):
        ...
        renderer = RendererFactory.create_renderer(
            self.root, self.config, preferred_type="graphviz"
        )
    ...
    def test_preferences_dialog_workflow(self, mock_engines, mock_available):
        ...
        with patch(
            "hbat.gui.graphviz_preferences_dialog.GraphVizPreferencesDialog"
        ) as mock_dialog:
            mock_instance = Mock()
    ...
    def test_visualization_window_workflow(self, mock_available):
        ...
        with patch("tkinter.Toplevel"):  # Mock the window creation
            viz_window = ChainVisualizationWindow(
                self.root, self.chain, "test_chain", self.config
            )
    ...
    def test_configuration_persistence_workflow(self, mock_available):
        ...
        try:
            with patch("pathlib.Path.home", return_value=Path(test_temp_dir)):
                ...
                self.assertEqual(
                    fresh_config.get_graphviz_export_dpi(), 300
                )  # Default DPI
                ...
                self.assertEqual(
                    new_config.get_graphviz_engine(), "dot"
                )  # Should default to dot
                self.assertEqual(
                    new_config.get_graphviz_export_dpi(), 300
    ...
    def test_renderer_switching_workflow(self, mock_engines, mock_available):
        ...
        for renderer_type, renderer_name in renderers:
            try:
                renderer = RendererFactory.create_renderer(
                    self.root, self.config, preferred_type=renderer_type
                )
    ...
    def test_integration_without_graphviz(self):
        ...
        with patch(
            "hbat.utilities.graphviz_utils.GraphVizDetector.is_graphviz_available",
            return_value=False,
        ):
            # Should fall back to matplotlib
                ...
                for renderer_type, renderer_name in renderers:
                    if renderer_name != "GraphViz":
                        try:
                            ...
                            if (
                                hasattr(old_renderer, "canvas_frame")
                                and old_renderer.canvas_frame
                            ):
