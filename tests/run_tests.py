...
def run_with_pytest(args):
    ...
    try:
        ...
        if hasattr(args, "unit") and args.unit:
            ...
            pytest_args.extend(
                ["-m", "integration", os.path.join(test_dir, "integration")]
            )
        ...
        if args.fast:
            ...
            pytest_args.extend(
                ["-m", "performance", os.path.join(test_dir, "performance")]
            )
...
def run_legacy_tests(args=None):
    ...
    try:
        ...
        if args and (args.gui or args.cli):
            print("Note: Legacy test runner does not support modular test selection.")
...
Examples:
  ...
  %(prog)s --legacy           # Use legacy test runner
    ...
    parser.add_argument(
        "--unit", action="store_true", help="Run only unit tests (fast, isolated)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-pdb", action="store_true", help="Skip tests requiring PDB files"
    )

    # Test execution options
    parser.add_argument(
        "--fast", action="store_true", help="Skip slow and integration tests"
    )
