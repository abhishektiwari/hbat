...
import json
import csv
from hbat.cli.main import detect_output_format, create_parser, run_analysis
...
class TestSingleFileExports:
    ...
    def test_export_to_json_single_file(self, mock_analyzer):
        ...
        try:
            ...
            mock_analyzer.get_summary = Mock(
                return_value={
...
class TestMultipleFileExports:
    ...
    def test_export_to_csv_files(self, mock_analyzer):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert os.path.exists(
                os.path.join(tmpdir, "results_cooperativity_chains.csv")
            )
            # No pi interactions file should be created since list is empty
            assert not os.path.exists(
                os.path.join(tmpdir, "results_pi_interactions.csv")
            )
    ...
    def test_export_to_json_files(self, mock_analyzer):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert os.path.exists(
                os.path.join(tmpdir, "results_cooperativity_chains.json")
            )
            ...
            with open(
                os.path.join(tmpdir, "results_cooperativity_chains.json"), "r"
            ) as f:
                data = json.load(f)
...
class TestCLIOutputIntegration:
    ...
    def test_output_format_detection_in_cli(self, sample_pdb_file):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "hbat.cli.main.NPMolecularInteractionAnalyzer"
