...
def test_ccd_bond_data_loading():
    ...
    assert load_time < 30.0, (
        f"Bond data loading took too long: {format_time(load_time)}"
    )
...
def test_ccd_atom_data_loading():
    ...
    assert load_time < 20.0, (
        f"Atom data loading took too long: {format_time(load_time)}"
    )
...
def test_phe_residue_bond_lookup():
    ...
    assert len(phe_bonds) >= 20, (
        f"PHE should have at least 20 bonds, found {len(phe_bonds)}"
    )
...
def test_phe_component_summary():
    ...
    assert phe_summary["atom_count"] >= 20, (
        f"PHE should have at least 20 atoms, found {phe_summary['atom_count']}"
    )
    assert phe_summary["bond_count"] >= 20, (
        f"PHE should have at least 20 bonds, found {phe_summary['bond_count']}"
    )

    # Summary generation should be fast
    assert summary_time < 0.01, (
        f"Summary generation too slow: {format_time(summary_time)}"
    )

    print(f"✅ Summary generation time: {format_time(summary_time)}")
    print(
        f"✅ PHE summary: {phe_summary['atom_count']} atoms, {phe_summary['bond_count']} bonds"
    )
...
def test_ccd_performance_comprehensive():
    ...
    print(
        f"   ⏱️  Total time for {len(test_residues)} lookups: {format_time(multiple_lookup_time)}"
    )
    ...
    print(
        f"   ⏱️  Total time for {len(phe_atoms)} atom lookups: {format_time(atom_lookup_time)}"
    )
    ...
    print(
        f"      Available atoms: {', '.join(phe_summary['atoms'][:10])}..."
    )  # Show first 10
    ...
    print(
        f"   🎯 Atom lookup performance: {format_time(avg_atom_lookup_time)} avg per atom"
    )
...
def main():
    ...
    try:
        test_ccd_performance_comprehensive()
        return 0
