...
class TestLargeStructurePerformance:
    ...
    def test_large_structure_analysis_performance(self, large_pdb_file):
        ...
        assert analysis_time < 300.0, (
            f"Analysis took too long: {analysis_time:.2f}s (should be < 5 minutes)"
        )

        # Verify substantial results for large structure
        summary = analyzer.get_summary()
        assert summary["hydrogen_bonds"]["count"] > 100, (
            "Large structure should have many H-bonds"
        )
        assert summary["total_interactions"] > 200, (
            "Large structure should have many interactions"
        )
        ...
        print(
            f"  Rate: {summary['total_interactions'] / analysis_time:.1f} interactions/second"
        )
    ...
    def test_large_structure_memory_usage(self, large_pdb_file):
        ...
        if memory_retained > memory_increase * 0.9:
            print(
                f"  Warning: Memory usage grew beyond peak (ratio: {memory_release_ratio:.2f})"
            )
        elif memory_retained < memory_increase * 0.5:
            print(
                f"  Good: Significant memory released (ratio: {memory_release_ratio:.2f})"
            )
        else:
            print(
                f"  Normal: Moderate memory retention (ratio: {memory_release_ratio:.2f})"
            )

        print(f"  Final memory: {final_memory:.1f}MB")
        print(f"  Memory retained: {memory_retained:.1f}MB")

    def test_large_structure_scalability_comparison(
        self, large_pdb_file, sample_pdb_file
    ):
        ...
        size_ratio = large_stats["total_interactions"] / max(
            small_stats["total_interactions"], 1
        )
        time_ratio = large_time / max(small_time, 0.01)

        # Performance should scale reasonably (not exponentially)
        efficiency_ratio = time_ratio / size_ratio
        assert efficiency_ratio < 15.0, (
            f"Performance scaling poor: {efficiency_ratio:.2f}x"
        )

        print("\nScalability Analysis:")
        print("  Small structure (6rsa.pdb):")
        print(f"    Time: {small_time:.2f}s")
        print(f"    Interactions: {small_stats['total_interactions']}")
        print("  Large structure (4jsv.pdb):")
...
class TestLargeStructureParameterPerformance:
    ...
    def test_parameter_strictness_performance_impact(self, large_pdb_file):
        ...
        assert (
            permissive_stats["hydrogen_bonds"]["count"]
            >= strict_stats["hydrogen_bonds"]["count"]
...
class TestLargeStructureOutputPerformance:
    ...
    def test_large_structure_json_export_performance(self, large_pdb_file):
        ...
        try:
            ...
            print(
                f"  H-bonds exported: {len(results['interactions']['hydrogen_bonds'])}"
            )
    ...
    def test_large_structure_summary_generation_performance(self, large_pdb_file):
        ...
        assert avg_summary_time < 0.1, (
            f"Summary generation too slow: {avg_summary_time:.3f}s"
        )
        assert avg_stats_time < 0.1, (
            f"Statistics generation too slow: {avg_stats_time:.3f}s"
        )
    ...
    def test_multiple_large_structure_analyses(self, large_pdb_file):
        ...
        for run in range(num_runs):
            ...
            assert stats["total_interactions"] > 100, (
                f"Run {run + 1} should find interactions"
            )

            print(
                f"Run {run + 1}: {run_time:.2f}s, {stats['total_interactions']} interactions"
            )
        ...
        assert time_variance < avg_time * 1.0, (
            f"Performance too variable: {time_variance:.2f}s"
        )
...
class TestPerformanceBenchmarks:
    ...
    def test_performance_baseline_documentation(self, test_pdb_dir):
        ...
        print("=" * 80)
        print(
            f"{'File':<15} {'Description':<15} {'Time(s)':<10} {'H-bonds':<10} {'Total':<10} {'Rate':<10}"
        )
        print("-" * 80)

        for filename, data in results.items():
            print(
                f"{filename:<15} {data['description']:<15} {data['time']:<10.2f} "
        ...
        for filename, data in results.items():
            if "Small" in data["description"]:
                ...
                assert data["time"] < 300.0, (
                    f"{filename} should complete within 5 minutes"
                )
                assert data["interactions"] > 100, (
                    f"{filename} should find many interactions"
                )

            # All should have reasonable interaction rates
            assert data["rate"] > 1.0, (
                f"{filename} should process at least 1 interaction/second"
            )
